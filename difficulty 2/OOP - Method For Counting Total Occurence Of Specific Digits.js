...
// and a second list,
// digits_list/digitsList that has the specific digits to count which length cannot be be longer than 10 (It's obvious, we've got ten digits).
...
function List () {
  this.countSpecDigits = function (integersList, digitsList) {
    let tuplesArr = []
    intergersListStr = integersList
      .map(interger => {
        return Math.abs(interger).toString()
      })
      .join('')

    digitsList.forEach(digit => {
      let strClone = intergersListStr
      tuplesArr.push([digit, strClone.split(digit.toString()).length - 1])
    })
    return tuplesArr
  }
}
...
var l = new List()
console.log(l.countSpecDigits([1, 1, 2, 3, 1, 2, 3, 4], [1, 3]))
